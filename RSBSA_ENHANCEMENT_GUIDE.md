...
### **Correct RSBSA Number Workflow:**
1. **DA Provides RSBSA Numbers** - Department of Agriculture assigns official RSBSA numbers
2. **Staff Manual Input** - Municipal staff manually enters DA-provided numbers into the system
3. **Validation Process** - Staff validates that RSBSA numbers are working in DA systems
4. **Tracking & Audit** - System tracks who assigned numbers and when

### **RSBSA Number Format (DA Standard):**
```
Example formats from DA:
- 10-43-24-001-000001 (with dashes)
- 1043240001000001 (without dashes)
- Custom DA formats as provided
```

**Note:** RSBSA numbers are NOT generated by the local system - they are provided by DA and manually entered by staff.
...
// RSBSA Number Management (DA-provided numbers)
POST   /api/rsbsa/{id}/assign-number     // Assign DA-provided RSBSA number
POST   /api/rsbsa/{id}/validate-number   // Validate RSBSA number in DA system
